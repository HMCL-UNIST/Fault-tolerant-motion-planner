...
for MC = 1:length(phi_deviation)
    ...
    wait = sprintf('PD Monte Carlo %d/100',MC);
    disp(wait)
    ...
    simulation = sim('scen1_PDproposed');
    time_sim = time_sim + time_fault;
    for i = 1:length(phi)
...
for MC = 1:length(phi_deviation)
    ...
    wait = sprintf('SMC Monte Carlo %d/100',MC);
    disp(wait)

    Q.init = [0 0 3 0 0 0  0 0 0 deg2rad(20+phi_deviation(MC)) 0 0]'; %xyz xyzvel pqr phi theta psi
    Q.initInput = [3.1441 3.1441 3.1441 3.1441]/cos(deg2rad(20+phi_deviation(MC)));

    simulation = sim('scen1_SMC');
    time_sim = time_sim + time_fault;
    ...
    simulation = sim('scen1_SMCproposed');
    time_sim = time_sim + time_fault;
    for i = 1:length(phi)
        err_norm_proposed(MC,i) = norm([phi(i)-peak_mp, theta(i)-deg2rad(0)]);
    end
...
for MC = 1:length(phi_deviation)
    ...
    wait = sprintf('BSC Monte Carlo %d/100',MC);
    disp(wait)

    Q.init = [0 0 3 0 0 0  0 0 0 deg2rad(20+phi_deviation(MC)) 0 0]'; %xyz xyzvel pqr phi theta psi
    Q.initInput = [3.1441 3.1441 3.1441 3.1441]/cos(deg2rad(20+phi_deviation(MC)));

    simulation = sim('scen1_BSC');
    time_sim = time_sim + time_fault;
    ...
    simulation = sim('scen1_BSCproposed');
    time_sim = time_sim + time_fault;
    for i = 1:length(phi)
        err_norm_proposed(MC,i) = norm([phi(i)-peak_mp, theta(i)-deg2rad(0)]);
    end
...
simulation = sim('scen2_PDproposed');
clearvars err_norm
time_sim = time_sim + time_fault;
for i = 1:length(phi)
    err_norm(i) = norm([x(i)-des_x, y(i)-des_y, z(i)-des_z]);
...
% SMC-based Proposed method
simulation = sim('scen2_SMCproposed');
clearvars err_norm
time_sim = time_sim + time_fault;
for i = 1:length(phi)
    err_norm(i) = norm([x(i)-des_x, y(i)-des_y, z(i)-des_z]);
...
% BSC-based Proposed method
simulation = sim('scen2_BSCproposed');
clearvars err_norm
time_sim = time_sim + time_fault;
for i = 1:length(phi)
    err_norm(i) = norm([x(i)-des_x, y(i)-des_y, z(i)-des_z]);
...
% PD-based Proposed Method
simulation = sim('scen3_PDproposed');

figure(29); hold on
ax29 = gca;
plot(x,y, 'b-')
axes('Position',[.51 .22 .3 .15])
box on; hold on; grid on;
ax29_enlarge = gca;
plot(x,y, 'b-')

% SMC-based Proposed Method
simulation = sim('scen3_SMCproposed');
...
% BSC-based Proposed Method
simulation = sim('scen3_BSCproposed');

figure(29)
axes(ax29);
plot(x, y, 'g-.','LineWidth',1.05)
...
axes(ax29);
plot(trajectory(:,1),trajectory(:,2),'ko','MarkerSize',6,'MarkerFaceColor','k');
...
obstacle13 = [35,30,5,6];
obstacle14 = [35,30,5,6];
obstacle15 = [35,30,5,6];
...
legend('Proposed strategy with PD','Proposed strategy with SMC','Proposed strategy with BSC','Waypoints given by RRT*','Initial position','Safe zone','Location','northwest')

xlim([-8 50]); ylim([-8 50]);

axes(ax29_enlarge);
xlim([22 25]); ylim([27 28.5]);